...
function placeNotes() {
	for (n = 1, refCount = $('a.side-matter-ref').length; n <= refCount; n++) { // Iterate through notes and position each
		...
		var refPosition = $(ref).positionRelative('.container').top; // Position of reference anchor
		var notePosition = $(note).positionRelative('.container').top; // Position of sidenote
